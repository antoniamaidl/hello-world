...
- ""
date: "2020-10-01"
description: Dual Class Share Arbitrage
draft: false
image: pic11.jpg
keywords: ""
slug: blog5
...
For this purpose we have selected a sample of 13 companies with dual listed share classes on the open market. We have downloaded daily share prices for both share classes for each company. As Under Armour was most recently listed in 2016, we will only consider data from that specific day onward. 

First, the data is cleaned, we select the columns needed for the analysis, change dates into date format with lubridate and then rename the columns as they are in an inconvenient format. Then we calculate the average for each day of the month for each company using the group_by function.
...
We made it! All data has been cleaned and daily averages have been calculated for each month. Now it's time to combine data frames to get an overall average. We use rbind to bind data frames together. "Do.call" is needed as we combine multiple data frames.

Then we create a new table. We again calculate averages across our data set with group_by. For the purpose to visualise our averages nicely, we need to combine day and month columns with the as.Date function. This will also keep the format as Date and not character. The formula automatically changes the year to 2020 - however, ignore 2020, these are averages.

The range function gives us an indicator of our data set, which can be useful for investors.
...
What a beautiful chart! Let's draw some conclusions:
